...
int32_t current_to_torque(int32_t curr_ma){
	float ret = 0;//curr_ma * CURRENT_FACTOR_mA;
	return ret;
...
static int16_t erpm_to_int16(int32_t erpm){
	int32_t out = 0; //((int32_t)HALL_M1._Super.DPPConvFactor * erpm) / ((int32_t) SPEED_UNIT * (int32_t)HALL_M1._Super.hMeasurementFrequency);
	return out;
}
...
float VescToSTM_get_pid_pos_now(){
	return 0;//360.0 / 65536.0 * (float)pMCI[M1]->pSTC->SPD->hElAngle;
}
...
float VescToSTM_get_temperature(){
	return 0;//NTC_GetAvTemp_C(pMCT[M1]->pTemperatureSensor);
}
...
/*	if(!pMCI[M1]->pFOCVars->Iq_samples || !pMCI[M1]->pFOCVars->Id_samples) return 0;
	...
	wAux1 = wAux1 * SIGN(iq);
*/
	return 0;//(float)wAux1 / CURRENT_FACTOR;

}

float VescToSTM_get_input_current(){
	return 0;// (float)pMPM[M1]->_super.hAvrgElMotorPowerW/(float)VBS_GetAvBusVoltage_V(pMCT[M1]->pBusVoltageSensor);
}
...
float VescToSTM_get_bus_voltage(){
	return 0; //(float)(VBS_GetAvBusVoltage_d(pMCT[M1]->pBusVoltageSensor)) / BATTERY_VOLTAGE_GAIN;
}

int32_t VescToSTM_get_erpm(){
	return 0;// MCI_GetAvrgMecSpeedUnit( pMCI[M1] ) * HALL_M1._Super.bElToMecRatio;
}

int32_t VescToSTM_get_erpm_fast(){
	return 0;// ( int16_t )( (  HALL_M1._Super.hElSpeedDpp * ( int32_t )HALL_M1._Super.hMeasurementFrequency * (int32_t) SPEED_UNIT ) / (( int32_t ) HALL_M1._Super.DPPConvFactor));
}

int32_t VescToSTM_get_rpm(){
	return 0;// MCI_GetAvrgMecSpeedUnit( pMCI[M1] );
}
...
int32_t VescToSTM_get_tachometer_abs_value(bool reset) {
	int val = 0;//HALL_M1.tachometer_abs;
...
float VescToSTM_get_duty_cycle_now(void) {
	...
	float amplitude = 0;//(MCI_GetPhaseVoltageAmplitude(pMCI[M1]) * SIGN(Vqd.q)) / 32768.0;
	return amplitude;
}
